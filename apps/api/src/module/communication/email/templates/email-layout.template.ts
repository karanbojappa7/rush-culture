import { brand, resolveTheme } from '@linq/site-config';
...
export function renderEmailLayout(input: EmailLayoutInput): string {
  const theme = resolveTheme(brand.themeId);
  const colors = {
    bg: theme.paper,
    panel: theme.panel,
    ink: theme.ink,
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:${colors.panel};border:1px solid ${colors.line};">
          ...
          <tr>
            <td style="padding:20px 28px 28px;border-top:1px solid ${colors.line};background:${colors.bg};">
              ${footerNote}
