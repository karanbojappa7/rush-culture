...
export class UserService extends BaseService {
  ...
  async create(payload: CreateUserDto): Promise<User> {
    ...
    const role = await this.roleService.findByCode(payload.roleCode ?? 'CUSTOMER');
    const userType = await this.userTypeService.findByCode(
      payload.userTypeCode ?? 'INDIVIDUAL',
    );
  ...
  async update(payload: { id: string; data: UpdateUserDto }): Promise<User> {
    ...
    if (payload.data.email !== undefined) data.email = payload.data.email;
